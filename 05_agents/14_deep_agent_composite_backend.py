...
agent = create_deep_agent(
    ...
    - 重要记忆：写入 `/store/` 目录（如 `/store/profile.txt`），保存到数据库。
    """
)
...
local_path = Path("./agent_workspace").resolve() / "local.txt"  # Path 拼接路径
...
found = any("memory.txt" in item.key for item in items)
if found:
    print("数据库记忆文件已找到")
else:
    print("数据库记忆文件缺失")